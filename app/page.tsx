...
function ProductPicture({ src, alt }: ProductPictureProps) {
  return (
    ...
    <li className="flex h-fit w-fit flex-col gap-[10px] rounded-[6.46px] border-[0.65px] border-[#DBDBDB] p-[10px]">
      <div className="relative h-[149px] w-[282px]">
        <div className="absolute h-full w-full rounded-[6.46px]">
          <ProductPicture src={image.src} alt={image.alt} />
        </div>
...
async function List() {
  ...
  return (
    <ul className="justify-center grid grid-cols-shop gap-[16px] p-10 px-[87px]">
      {data.map((item) => (
        <Card
          id={item.id}
          name={item.name}
...
function Skeleton() {
  return (
    <li
      role="status"
      className="flex h-fit w-fit flex-col gap-[10px] rounded-[6.46px] border-[0.65px] border-[#DBDBDB] p-[10px] animate-pulse"
    >
